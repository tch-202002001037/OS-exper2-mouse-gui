...
int get_stucked(int posx, int posy) {
    ...
    for (pos = 0; pos < obstacle_count; ++pos) {
        if (posx - (BIRD_WIDTH / 2) > obstacles[pos].xpos2) break;
        if (posx + (BIRD_WIDTH / 2) < obstacles[pos].xpos1) continue;
        if ((posx - (BIRD_WIDTH / 2) >= obstacles[pos].xpos1 && posx - (BIRD_WIDTH / 2) <= obstacles[pos].xpos2)
         || (posx + (BIRD_WIDTH / 2) >= obstacles[pos].xpos1 && posx + (BIRD_WIDTH / 2) <= obstacles[pos].xpos2)
         || (posx - (BIRD_WIDTH / 2) >= obstacles[pos].xpos1 && posx + (BIRD_WIDTH / 2) <= obstacles[pos].xpos2)) 
        {
            if ((posy - (BIRD_HEIGHT / 2) >= obstacles[pos].ypos1 && posy - (BIRD_HEIGHT / 2) <= obstacles[pos].ypos2)
             || (posy + (BIRD_HEIGHT / 2) >= obstacles[pos].ypos1 && posy + (BIRD_HEIGHT / 2) <= obstacles[pos].ypos2)
             || (posy - (BIRD_HEIGHT / 2) >= obstacles[pos].ypos1 && posy + (BIRD_HEIGHT / 2) <= obstacles[pos].ypos2)) 
            {
                return 1;
            }
...
int main() {
    ...
    obstacle_count = rand() % 20 + 1;
    for (i = 0; i < obstacle_count; ++i) {
        obstacles[i].xpos1 = 200 + i * 50 - (BIRD_WIDTH / 2);
        obstacles[i].xpos2 = 200 + i * 50 + (BIRD_WIDTH / 2);
        if (rand() % 2)
            obstacles[i].ypos1 = 0, 
            obstacles[i].ypos2 = rand() % 100 + 1;
        else 
            obstacles[i].ypos1 = rand() % 100 + 100,
            obstacles[i].ypos2 = 199;
    }
    /*
    */
#ifndef DEBUG
    ...
    while(1) {
        ...
        for (i = 0; i < obstacle_count; ++i) 
            paint(obstacles[i], 3),
            obstacles[i].xpos1 -= 20, 
            obstacles[i].xpos2 -= 20, 
            paint(obstacles[i], 12);
    }
    if (ok) _GAME_OVER();
    return 0;
